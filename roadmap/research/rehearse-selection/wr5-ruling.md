...
**WR5's discovery query returned eight results and the deliverable reports one.**

**OBSERVED** — `GET api.safe.global/tx-service/eth/api/v1/safes/0x2aCA71.../multisig-transactions/?executed=true&to=0x1a44076050125825900e736c501f859c50fE728c` returns `count: 8`. WR5 §2 cites this exact filter and reports only nonce 714 (its candidate A). The other seven include **four transactions that WR5 never mentions and that are the only Safe executions in the entire pool touching a `ROUTE-ETH-OP-v1` participant OApp**:
...
- **Isolation: strongest in the pool, and measured correctly.** **OBSERVED** full-block scan of all 198 top-level `to` values against {Endpoint, OFT Adapter, controller Safe, SendUln302, ReceiveUln302}: the target is the **only** match. Critically, the property that matters is *nothing after index 52*, which holds.
...
**R2 — same Safe → `setConfig(L1 OFT Adapter, SendUln302, [(30111, ULN)])`**, block 22099706 (`0x609d97cc…`, parent `0x7a92e5d6…`, ts `0x67de2cfb`, baseFee `0x17bc6efb`, 122 txs, **index 47**). **OBSERVED** two *prefix* transactions (indices 36, 37) call ReceiveUln302 — a different contract from the one R2 writes, and both before the target, so actual-effect isolation is intact while the naive "unique caller" story is not. Equally rehearsable; slightly noisier block.
...
Why it wins on every axis the gate scores:
...
- **52-tx prefix, clean isolation, verified by me, not inherited.**
...
| Finding | Bearing |
|---|---|
| WR5 reported 1 of 8 results from its own discovery query | Selection is not defensible; the pool is not the pool |
...
| 7 | OP timelock execution stream never searched; pool must not be a closed 4-address universe | **Open, and the cost is now demonstrated** — the miss was on the L1 side too (§0). |
...
| **G8** | **Discovery method misses batched actions.** A `to=<target>` filter cannot see MultiSend-wrapped route actions; **OBSERVED** the L1 Safe has **147** executed MultiSend batches. The 26Q2 SecurityUpgrade (unpause + rate limits + `setConfig` in one atomic batch, hitting **four** matrix rows) is almost certainly in there — the ideal batch-structure fixture. | Enumerate all 147 MultiSend executions, decode each inner call's `to`, and select those whose inner targets intersect {L1 OFT Adapter, Endpoint}. Complement with an **event-first** search: `UlnConfigSet` / `PeerSet` / rate-limit events on the route contracts, then walk back to the transaction. Event-first is order-independent and immune to both the `to`-filter and the relayed-execution blind spots. |
