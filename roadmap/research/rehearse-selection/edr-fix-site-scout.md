...
- `crates/test/block_replay/src/lib.rs:377-385` ~~asserts~~ **debug-asserts** (`debug_assert_eq!` — Codex correction) full execution-receipt equality (for deposits that includes `deposit_nonce`, per `crates/edr_op/src/receipt/execution/deposit.rs:10-28`).
...
- Instantiated for OP mainnet at `crates/edr_op/tests/integration/full_block.rs:11-82` — Regolith `105_235_064`, Canyon, Ecotone `121_874_088`, Granite, Holocene `130_423_412`/`+1`, Isthmus+1, Jovian. ~~All contain an index-0 L1-attributes deposit at real forked state where the depositor's nonce is far from zero.~~ **SUPERSEDED (Codex verdict): the protocol guarantees an index-0 L1-attributes deposit, but the individual depositor nonces are UNVERIFIABLE from the workspace (block numbers only, no cached tx data; tests need remote RPC). "Far from zero" was unsupported.**
...
INFERRED: those tests currently pass (not run here — they need remote RPC). ~~If so, execution + receipt building are already canonical for deposits at nonzero account nonce, and **the mempool is the only thing standing between EDR and byte-exact OP block replay over JSON-RPC.**~~ **SUPERSEDED (Codex verdict, REFUTED): if green they corroborate the execution/receipt path, but the mempool is NOT the sole blocker on the JSON-RPC path — automine fee validation (`data.rs:1664-1692`, esp. `:1675-1681` rejecting zero-fee deposits at positive base fee), mint-unaware balance validation (`mem_pool/src/lib.rs:501-507`), and miner priority arithmetic (`miner/src/lib.rs:611-612`, `0 - base_fee` overflow risk) remain material blockers.**
...
**Not** available at the check site: `MemPool<SignedTransactionT: ExecutableTransaction>` (`crates/mem_pool/src/lib.rs:206`) is chain-agnostic, and `crates/mem_pool/Cargo.toml` depends ~~only~~ on `edr_chain_spec`, `edr_primitives`, `edr_state_api`, `edr_transaction`, `indexmap`, `thiserror` **plus optional `tracing` (`Cargo.toml:13` — Codex correction)**. A case-insensitive `grep deposit` over `crates/edr_provider`, `crates/mem_pool`, `crates/block` returns **zero** `.rs` hits — that whole layer is deposit-unaware.

Established precedent for exactly this: the marker traits `IsEip155` / `IsEip4844` / `IsLegacy` / `IsSupported` at `crates/edr_transaction/src/lib.rs:196-219` (~~impls for OP at `crates/edr_op/src/transaction/signed.rs:151-170`~~ **Codex correction: `data.rs:2537` calls `is_eip4844()` on the transaction *type*; the directly consumed OP impl is `crates/edr_op/src/transaction/type.rs:33-37`**), consumed by generic provider code at `crates/edr_provider/src/data.rs:2537`. `edr_mem_pool` **already depends on `edr_transaction`**, so a sibling `IsDeposit` trait is in scope at the check site with no new dependency edge. — INFERRED (design), OBSERVED (the deps and precedent).
...
5. **Mine ordering.** Deposits hardcode `gas_price() == 0` (`crates/edr_op/src/transaction/signed/deposit.rs:77-81`). Under `MineOrdering::Priority` the comparator (`crates/block/miner/src/lib.rs:623-641`) sorts by effective miner fee → ~~a deposit sorts **last**, whereas OP consensus requires deposits first~~ **SUPERSEDED (Codex verdict): `effective_miner_fee` computes `0 - base_fee` at `miner/src/lib.rs:611-612` — at positive base fee this can overflow/panic in checked builds; in wrapping builds the zero effective fee ranks behind positive-fee transactions but not necessarily absolutely last (ties break on `order_id`). OP consensus requires deposits first either way.** `MineOrdering::Fifo` (`:269`, `:616-621`) preserves submission order. Also `crates/block/miner/src/lib.rs:280-283` drops the caller entirely if `min_gas_price > 0`.
...
8. **Automine fee validation blocks zero-fee deposits (Codex finding).** After the nonce gate, `validate_auto_mine_transaction` runs generic fee checks at `crates/edr_provider/src/data.rs:1664-1692`; deposits expose `max_fee_per_gas = 0`, so `:1675-1681` rejects them whenever the next base fee is positive. Bypassing `:1648` alone does not fix automine.
9. **Balance validation is mint-unaware (Codex finding, omitted from the original scout).** `MemPool` checks generic `upfront_cost` at `crates/mem_pool/src/lib.rs:501-507` — includes `value`, excludes a deposit's `mint`. Value-bearing deposits can be wrongly rejected for insufficient prestate balance even after a nonce guard. (The index-0 L1-attributes deposit is normally zero-value, so canonical replay of that tx is unaffected.)

**Assessment:** ~~a guard at `crates/mem_pool/src/lib.rs:494` + `crates/edr_provider/src/data.rs:1648` is a genuine, small fix that unblocks the canonical-replay use case in its normal shape (≤1 deposit per sender per block, submitted then mined).~~ **SUPERSEDED (Codex verdict, REFUTED): a nonce guard is only a partial fix. A narrow automine-off replay may work if the deposit is zero-value or sufficiently funded, there is at most one per sender, no `update()` intervenes, `min_gas_price` is zero, and mining preserves canonical ordering. A general fix must also address automine fee validation, mint-aware funds validation, replacement/update semantics, and miner ordering.** The architecturally clean version — keying deposits by `sourceHash` and keeping them out of the nonce-ordered queues, the way op-geth keeps deposits out of the txpool and injects them from payload attributes (externally corroborated by Codex against op-geth `core/txpool/validation.go` and the OP derivation spec's `noTxPool=true`) — is a larger refactor of `MemPool`'s internals. Both are defensible; that's the maintainers' call.
...
## (d) DRAFT follow-up comment for issue #1578 — SUPERSEDED by (d′)

**SUPERSEDED 2026-07-26 by the Codex verdict (NEEDS-CORRECTIONS, see
`edr-scout-codex-verdict.md`). Retained verbatim below for the record; the postable version
is (d′). Do NOT post this version.**
...
> **Follow-up: I read the source to try to locate the check, in case it's useful.**
>
> I traced this at commit `c66cd68aaa3cd7a5af1968a2933fe7bc5db9f809`. I also checked that every source file cited below is byte-identical between that commit and the `@nomicfoundation/edr@0.14.2` tag (`375a455277e60040cc023d1a666a80b17ef7f142`), so the line numbers should apply to the release I tested as well. You know the architecture far better than I do, so please treat all of this as a report rather than a diagnosis.
>
> **Where the message comes from.** The string is EDR's own, not revm's: `MemPoolAddTransactionError::NonceTooLow` at `crates/mem_pool/src/lib.rs:132-141`, raised in `MemPool::add_transaction` at `crates/mem_pool/src/lib.rs:493-499`, surfaced as `-32000` via `crates/edr_provider/src/error.rs:528`.
>
> **How a deposit reaches it.** `provider.rs:331-334` → `requests/eth/transactions.rs:184` (`handle_send_raw_transaction_request`) → `data.rs:2532` (`send_transaction`) → `data.rs:1291` (`add_pending_transaction`) → `data.rs:1299` → the check. `validate_send_raw_transaction_request` (`transactions.rs:325`) doesn't check nonces, so the mempool is the first nonce gate. Note there's a second one at `data.rs:1648-1662` (`validate_auto_mine_transaction`) that fires first when automining is on — I hit the mempool one because my harness mines manually. And bypassing `data.rs:1648` alone would not fix the automine path: the generic fee checks at `data.rs:1664-1692` (particularly `:1675-1681`) still reject a zero-fee deposit whenever the next base fee is positive.
>
> The value being compared is the deposit's hardcoded placeholder at `crates/edr_op/src/transaction/signed/deposit.rs:95-100`, whose own comment notes it isn't a real nonce.
>
> **The half I got wrong in the original report.** I asked for `depositNonce` to record the depositor's nonce at execution — that's already implemented, at `crates/edr_op/src/receipt/execution.rs:106-115`, reading `pre_execution_state.basic(caller).nonce`. I only saw `0x0` because my workaround forced the account's nonce to `0`. So no receipt-side change appears to be needed at all.
>
> What made this click: `crates/test/block_replay/src/lib.rs:225-227` feeds canonical transactions straight into `builder.add_transaction(...)`, skipping the mempool entirely, then asserts full header equality (including `receiptsRoot`) at `:388` and debug-asserts full execution-receipt equality at `:377-385`. `crates/edr_op/tests/integration/full_block.rs` runs that over OP mainnet blocks from Regolith through Jovian. I haven't run those tests (they need remote RPC), so I can't speak to their current pass status — but if they pass, they corroborate that the execution and receipt paths are already canonical for deposits; they don't by themselves show the mempool is the only remaining issue on the JSON-RPC path.
>
> **The shape a fix might take.** `MemPool` is generic over `ExecutableTransaction` and `edr_mem_pool` doesn't depend on `edr_op`, so there's no deposit discriminant in scope at `lib.rs:494` today. It does already depend on `edr_transaction`, where `IsEip4844` / `IsLegacy` / `IsSupported` live (`crates/edr_transaction/src/lib.rs:196-219`) — and `data.rs:2537` already uses exactly that pattern (via the transaction-type impl at `crates/edr_op/src/transaction/type.rs:33-37`) to special-case blob transactions in generic provider code. A sibling predicate consulted at `mem_pool/src/lib.rs:494` and `data.rs:1648` would follow the existing grain.
>
> **Where it's more than a guard clause, in case it matters for scoping.** Several other places in the same layer assume fee/nonce ordering:
> - `data.rs:1664-1692` — the automine fee validation above; a nonce exemption alone leaves deposits rejected there.
> - `lib.rs:501-507` — the upfront-cost balance check includes `value` but not a deposit's `mint`, so a value-bearing deposit that's funded by its own mint can still be rejected for insufficient balance.
> - `lib.rs:538-556` — replacement detection keys on `(sender, nonce)`. Two deposits from one sender both read nonce `0`, so the second looks like a replacement, and `validate_replacement_transaction` (`:658-690`) passes trivially since both gas prices are `0`.
> - `lib.rs:374-383` + `:403-415` — `update()` drops anything with `nonce < sender.nonce`, and `update()` runs after every mined block (`data.rs:1480-1482`) and on `hardhat_setBalance`/`setNonce` (`data.rs:1026`, `:1184`). A queued-but-unmined deposit wouldn't survive.
> - `crates/block/miner/src/lib.rs:611-612` — priority ordering is also unsafe for deposits: `effective_miner_fee` subtracts the positive base fee from the deposit's zero max fee, which may overflow in checked builds; otherwise the resulting zero fee ranks behind positive-fee transactions, while OP wants deposits first. `Fifo` preserves submission order.
>
> So a nonce guard is only a partial fix: a narrow automine-off replay may work if the deposit is zero-value or sufficiently funded, there's at most one per sender, no `update()` intervenes, and `min_gas_price` is zero with FIFO ordering — but a general fix must also address automine fee validation, mint-aware funds validation, replacement/update semantics, and miner ordering. The fully robust version probably means keying deposits by `sourceHash` and keeping them out of the nonce-ordered queues entirely, closer to how op-geth keeps deposits out of the txpool. I don't have a view on which you'd prefer.
>
> If the guard-style direction is acceptable as a first step, I'm happy to submit a PR — the admission/automine guards plus a mempool unit test and an `eth_sendRawTransaction` deposit-admission test under `crates/edr_op/tests/integration/`, with a changeset. (`crates/test/mem_pool/src/lib.rs:13-18` is currently hardcoded to `L1SignedTransaction`, so that fixture would need to be generic or get an OP sibling.) And equally happy to leave it with you if the deeper restructuring is the right call — just let me know either way.

## (e) PR complexity estimate

~~**S–M.**~~ **M (re-estimated after the Codex verdict).** The nonce guard itself is ~10 lines across two call sites, but the verdict established that a defensible minimal PR must also exempt deposits from the automine fee checks (`data.rs:1664-1692`) — so the true minimal surface is three sites plus threading a chain-agnostic deposit predicate (new trait in `edr_transaction` + impls for every `SignedTransaction` type + generic bounds on `MemPool::add_transaction` and `ProviderData::send_transaction`) and making the L1-hardcoded `MemPoolTestFixture` generic. **Main risk:** even that leaves latent assumptions in `MemPool` (mint-unaware balance check, same-sender deposit replacement, `update()` eviction, priority-ordering overflow) — so maintainers may reasonably want the larger `sourceHash`-keyed restructuring instead, which would turn this into an L.
