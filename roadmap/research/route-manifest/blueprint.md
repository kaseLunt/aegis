...
| S2 | `identity.targets` | Six identity-bearing contracts per the engine's `targets[]` shape: L1 OFT Adapter (eip1967), OP OFT (eip1967), L1 SendUln302, L1 ReceiveUln302, OP SendUln302, OP ReceiveUln302 (direct), plus EndpointV2 on both chains (direct) — each with `identityStrategy`, `expectedImplementation` where proxied, `expectedRuntimeCodeHash` | Identity (both directions); every other cell transitively — ABI selection is code-hash-gated (spec §Deployment code identity) |
...
| `expectedRuntimeCodeHash` for **all** of the above | — | **GAP G-01. Schema-mandatory; blocks the Identity row outright.** |
...
|---|---|---|---|
| **G-01** | `expectedRuntimeCodeHash` for every S2 target; impl→pinned-commit build mapping | **Identity row, and transitively every ABI-dependent cell** (schema-mandatory field) | An observed-side lane (not me) fetches `eth_getCode` at a finalized block via 2 independent providers and sha256-hashes; a separate reviewer binds each hash to the pinned commit's build artifacts. Three roles, three parties — D-006. |
