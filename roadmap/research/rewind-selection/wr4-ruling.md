...
- **Ordering has exactly one source: the walk.** ~~"`seq` plus the `prevEntryHash` chain"~~ — **withdrawn (round 4): two sources with no conflict rule is not an ordering, it is an ambiguity, and it left a coherently rehashed nonmonotonic register able to support contradictory orders while staying approved-reachable.** `position(e)` is the hop count from genesis to `e` along `prevEntryHash`, walked backwards from `approvedSupersessionRegisterHead`. Nothing else orders a record — not `seq`, not `detectedAt`, not file line order, not block number.
  - **Genesis rule.** Exactly one entry in the file may carry `prevEntryHash: null`, and the walk from the approved head MUST terminate at it. Terminating on a dangling pointer, or meeting a second null-prev entry, refuses the **whole file** (`register_genesis_invalid`) — never a partial load.
  - **Single-path rule.** Every entry lies either on the walk from the approved head or on its single unbroken continuation past it. Entries past the head are **pending**, not orphans (records legitimately land before the head is re-approved). An entry on neither is an orphan — a discarded branch — and refuses the whole file (`register_orphan_entry`). Silently skipping an orphan is precisely the behaviour this record exists to prevent.
  - **`seq` is display-only and never a tiebreaker.** If present it MUST equal `position(e)`; disagreement **refuses** the file (`register_seq_desync`). The conflict rule stated flatly: **the walk always wins, and a disagreement never resolves in `seq`'s favour — it refuses.** Omitting `seq` entirely is the safer authoring choice and is explicitly permitted.
...
- **Record admissibility — its own state vocabulary, not the manifest's.** ~~"A record not reachable from the approved head yields `unknown` with a typed reason — the untrusted-manifest path reused (`trust.ts:214-219`)"~~ — **withdrawn (round 4): wrong citation and wrong state class.** OBSERVED corrections: `trust.ts:213-219` is the **load-time content-hash integrity** path (recompute `manifestContentHash(raw)`; mismatch **throws** `ManifestError("integrity_mismatch","/contentHash")`). Approved-set evaluation is `evaluateTrust` at **`trust.ts:266-278`**, returning `state: "trusted" | "untrusted"` with `reasonCodes ["approved_hash"] | ["manifest_hash_not_approved"]` — a **`PolicyTrustState`** (`ENGINEERING_SPEC:166`, enum-guarded `canonical.ts:434`), a different type from `VerificationState` (`ENGINEERING_SPEC:150-156`). And an untrusted manifest emits **no** `unknown` verifications at all: `loaded` is non-null only when trusted (`trust.ts:299-308`), `targetsOf(null)` returns `[]` (`engine.ts:293-296`), so nothing is verified (`engine.ts:199-201`). The supersession mapping therefore stands alone, in two levels:
  ...
  2. **Result state — about anything that cites the affected observation.** A non-`anchored` record supplies **no supersession**: the earlier observation is not annotated and no sealed artifact changes (test 1 holds regardless). But un-annotated MUST NOT read as "still current." So for any verification **newly evaluated** whose evidence cites the affected observation, `VerificationState` = **`"unknown"`** (`ENGINEERING_SPEC:150-156`) with reason `supersession_unanchored`, plus a `limitation` on the rendering surface. Why this differs from the manifest case, stated because the disanalogy is the point: there, an untrusted manifest removes the *targets*, so there is nothing to verify and zero verifications exist; here the claim already exists and only its lineage status is undetermined. The honest state for a determinable-but-undetermined predicate is `unknown` — never `pass`, never `fail`, and not `not_applicable` (the supersession *is* applicable; it merely cannot be admitted).
  3. **Never retroactive.** Both levels attach to the *evaluation* and the *timeline rendering*, never to an already-sealed report; a sealed report's bytes and `reportHash` are immutable (test 1).
...
- **R11** a record is quotable as an asserted supersession only when its register entry is reached by the walk from `approvedSupersessionRegisterHead`; otherwise `registerTrust.state` is `unanchored` (or `invalid`) with a typed reason, no supersession is applied, and any newly evaluated verification citing the affected observation is `unknown`/`supersession_unanchored`. Never a silent apply. (Round-4: the bare "outcome is `unknown`" wording is superseded by the two-level mapping in the admissibility bullet above — the *record* takes a register-trust state, the *result* takes a `VerificationState`; they are different type classes and conflating them was the round-3 error.)
...
- **R14** `seq`, if present, must equal `position(e)`; disagreement is `register_seq_desync` and refuses the file. A `seq` conflict is never resolved in `seq`'s favour — the walk wins by construction, and the disagreement is reported as tamper evidence.
